...
function validate_jwt($cf_auth_jwt, $keys, $auth_domain)
{
    ...
    if (count($parts) !== 3) {
        error_log('AutoLoginWithCloudflare: Invalid JWT format - expected 3 parts, got ' . count($parts));
        return null;
    }
    ...
    if (!is_array($jwt_header)) {
        error_log('AutoLoginWithCloudflare: Failed to decode JWT header');
        return null;
    }
    ...
    // Fallback: try all keys (handles v7's stricter key size validation)
    if (!$jwt_decoded && $keys) {
        foreach ($keys as $key) {
            try {
...
function login()
{
    ...
    if (!$jwks || !isset($jwks['keys']) || !is_array($jwks['keys'])) {
        error_log('AutoLoginWithCloudflare: Invalid or empty JWKS structure. Aborting login process.');
        return;
    }
    ...
    try {
        ...
        error_log('AutoLoginWithCloudflare: Login error - ' . $e->getMessage());
    }
}
